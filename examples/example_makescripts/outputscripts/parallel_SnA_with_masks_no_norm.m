...
parpool(jm, 16);
job = createJob(jm);
job.AttachedFiles = {'~/pyp_run10/sna/release/2.0/core',[param.workerattach]};
...
command = ['cp -p ' param.share_directory fileName_wildcard ' ' param.local_destination];
%command = ['mv ' param.share_directory fileName_wildcard ' ' param.local_destination];
system(command);
