...
1) download raw data from: https://www.dropbox.com/sh/x28kfw0dg56r7hp/AAD1YeBsRbD07PylMxsp50eTa?e=2&dl=0   // dataPYP_femto corresponds to light state

2)put raw data in dataPYP. 

3)edit makescripts/make_all.scripts.sh and run. Variables including file path and input matrix can be changed on make_all.scripts.sh. Copy scripts from makescripts/outputscripts to NLSA and sna/release/2.0/variations

4)submit job files (.pbs) on compute server.

5)After adding relevant scripts and input data. base_pyprun can be copied directly to user's home directory (~/) on server
